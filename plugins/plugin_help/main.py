...
class PluginHelpTab(PluginTab):
    ...
    def get_help_text(self):
        ...
        <ul>
            ...
            <li>Seu plugin <strong>deve possuir um arquivo <code>main.py</code></strong> que servirá como ponto de entrada.</li>
...
class MeuPlugin(PluginTab):
    ...
    def __init__(self, parent=None):
        ...
        <h2>3. Estrutura do Arquivo <code>main.py</code></h2>
        <p>Além do arquivo do plugin, é necessário criar um <strong>main.py</strong> no diretório do plugin. Este arquivo deve:</p>
        <ul>
            <li>Definir a variável global <code>plugin_class</code> apontando para a classe do plugin.</li>
            <li>Opcionalmente, incluir uma função <code>main()</code> para testar o plugin de forma independente.</li>
        </ul>
        <pre style="background-color: #f4f4f4; padding: 10px;">
import sys
import os
...
# Função para teste independente do plugin
def main():
    ...
    main()
        ...
        <ol>
            ...
            <li>
                ...
                <p>
                    O botão <strong>"Atualizar Lista de Plugs"</strong> não só atualiza a lista de plugins e registra os plugins válidos no arquivo de configuração (<code>loaded_plugins.json</code>), como também reinicia o aplicativo automaticamente para carregar as novas configurações.
                </p>
            </li>
            <li>
