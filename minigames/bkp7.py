...
class ProgrammingClickerGame(QDialog):
    ...
    def init_ui(self):
        ...
        self.available_upgrades = []  # Lista de upgrades disponíveis
    ...
    def display_upgrades(self):
        """Exibe os upgrades carregados na interface."""
        for upgrade in self.available_upgrades:
            button = QPushButton(f"{upgrade['name']} - {upgrade['description']} (Custo: {upgrade['cost']} linhas)")
    ...
    def do_prestige(self):
        ...
        pet_xp_reward = (self.lines_of_code // points_per_xp) * xp_per_interval

        points_per_coin = self.coin_reward_interval["points"]
        coins_per_interval = self.coin_reward_interval["coins"]
        pet_coins_reward = (self.lines_of_code // points_per_coin) * coins_per_interval
